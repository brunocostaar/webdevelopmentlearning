...
class CafeForm(FlaskForm):
    ...
    location = URLField('Location (Maps URL)', validators=[DataRequired()])
    opentime = StringField('Opening time', validators=[DataRequired()])
    closetime = StringField('Closing time', validators=[DataRequired()])
    coffeerating = SelectField('Coffe rating', choices=["☕️", "☕️☕️", "☕️☕️☕️", "☕️☕️☕️☕️", "☕️☕️☕️☕️☕️"], validators=[DataRequired()])
...
def add_cafe():
    ...
    if form.validate_on_submit():
        with open("cafe-data.csv", mode="a", encoding='utf-8', newline='\n') as csv_file:
